...
const int M = 16; 
const int N = 1<<L; 
mint dp[M+1][N]; // till i, G_j's min in a.
...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int msk = 0; msk < MSK; msk++) {
            dp[i+1][msk] += dp[i][msk]; // not use x as min
            for (int j = 0; j < n; j++) {// as G_j's min if possible
                int k = 1<<j;
                if (msk & k) continue;
                int rem = MSK-x-msk;
                int ned = k - 1;
                dp[i+1][msk | k] += dp[i][msk] * c.c(rem, ned) * c.fac[k];
            }
        }
    }
    mint res = 0;
    for (int msk = 0; msk < MSK; msk++) {
        dp[m][msk] *= c.fac[MSK-msk-1];
        (__builtin_popcount(msk)&1) ? (res-=dp[m][msk]) : (res+=dp[m][msk]);
    }
    res *= MSK; // 0's pos
    cout << res;
}
