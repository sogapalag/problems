...
// c(p,q)2^q, q appear, x_1..q + y_1..r = n, w.r.t  x>0, y>=0
// for even, x_0 cnt=0/1. i.e. ...=n/n-1
void solve() {
    int k,n;
    cin >> k >> n;
    Comb c(n+k);
    ...
    for (int i = 2; i <= k+1; i++) {
        if (i&1) {
            ...
            mint sum = 0;
            for (int q = 0; q <= p; q++) {
                sum += c.c(p, q)*pw[q] * c.z(n-q, q+r);
            }
            res[i] = res[2*k+2-i] = sum;
        } else {
            int p = (i-1)/2;
            int r = k-1 - 2*p;
            mint sum = 0;
            for (int q = 0; q <= p; q++) {
                sum += c.c(p, q)*pw[q] * c.z(n-q, q+r);
                sum += c.c(p, q)*pw[q] * c.z(n-1-q, q+r);
            }
            res[i] = res[2*k+2-i] = sum;
        }
    }
