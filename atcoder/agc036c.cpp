...
// valid iff sum = 3m, max <= 2m, #odd <= m.
//
// calc, first relax max, c(n, odd) * c((3m-odd)/2 + n-1, n-1).
// fixed p_0 >= 2m, then sum = m , (delta)p_0>0, else >=0. with odd condition.
//    relax as p_0 >=0, minus p_0 = 0.
void solve() {
    int n, m;
    cin >> n >> m;
    int _n = 3*m/2 + n + 100;
