...
void solve() {
    ...
    for (int i = 4; i <= n; i++) {
        for (int j = 1; j < i-2; j++) {
            // 1 don't line. 2-i line or not.
            dp[i][j] += dp[i-1][j] + dp[i-1][j-1];
            // 1-x
            for (int x = 3; x < i; x++) {
                // 1..x has l. since x is smallest-1. so similar to above, 2-x line or not
                // x..n right part is same sub problem
                for (int l = 0; l < j; l++) {
                    dp[i][j] += (x>3?(dp[x-1][l] + (l?dp[x-1][l-1]:0)):(l==0?1:0)) * dp[i+2-x][j-1-l];
                }
            }
        }
    }
...
// O(n). https://oeis.org/A033282
void solve2() {
    int n,k;
    cin >> n >> k;
    Comb c(n+k);k--;
    
    mint res = c.c(n-3, k) * c.c(n+k-1,k) * c.inv[k+1];
    cout << res;
}
int main() {
