...
int main() {
    ...
    if (n == 0) {
        cout << z;
        cout << ".0000" + string(prec, '0');
        return 0;
    }
    // [1/8 *1/k] .... [1/8k...1/8k] [1/8] [1/4] [1/2] , 61pts
    // i.e. first op take some avg, remain exactly remain-ops to merge one-by-one
    // TODO correct method: https://wenku.baidu.com/view/7842de6784868762cbaed52e.html?pn=51
    // optimal-form: ... [len1]>= [len2] >=..>= [lenm]
    //   key optimization: #len>1 <= 14. monoqueue slope dp with double, then recover with decimal.
    //
