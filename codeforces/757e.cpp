...
// first note f_r+1 = zeta(f_r)
// so f_r = z^r (f0). and note z^r is count multi-chain.
//  thus, for n down to d, pass r edges, for factor(n/d), each prime'exp, diribute on which edges. i.e. c.z
// about 3000ms, TLE, if do some constant-optimization, may pass.
void solve() {
    int r, n;
    //cin >> r >> n;
    r = rng()%N; n = rng()%N;if(!n)n++;
    mint res = 0;
    auto calc = [&](int d){
        mint coeff = 1;
        for (int e: es[d]) {
...
// above painful part is iter ds[n]
// notice that, we only need es, since each prime view independent.
// its e 's any distribution corresponding one coeff. (sum coeff) * (sum coeff).. are exactly ways
// precalc dp table. dp[r][e]:= sum coeff of dist of e to r edges.
// transition, consider first edge weight 0 or not. 0, dp[i-1][j]; not implies at least 1, dist remain dp[i][j-1]
void __solve() {
    int r, n;
    cin >> r >> n;
    //r = rng()%N; n = rng()%N;if(!n)n++;
...
int main() {
    ...
    while(t--)__solve();
    clock_t y = clock();
    cerr<<(double)(y-x)/CLOCKS_PER_SEC;
    cout << endl;
}
