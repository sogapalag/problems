...
void solve() {
    ...
    cin >> n >> k;
    Comb c(n);
    vector<mint> dp(n+1); // #perm of [len]. \E early taken
    mint suff = 0;
    for (int len = k+1; len <= n; len++) {
        // max in first len-k ? 
        dp[len] = c.fac[len-1] * (len-k) + c.fac[len-1] * suff;
        suff += dp[len] * c.ifac[len];
        suff -= dp[len-k] * c.ifac[len-k];
    }
    mint res = 0;
    for (int i = k+1; i < n; i++) {
        res += dp[i] * c.a(n-1, n-i-1);
    }
    cout << res;
}
