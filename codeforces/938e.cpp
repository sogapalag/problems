...
// all >= should be right, and at least one is >
// res = n! sum_x x/(n-k). by simplify
// or quickly argument, for set {x, >=x}, x should occur be first, aka prob= 1/(n-k)
void solve() {
    int n; cin >> n;
    vector<int> a(n);
    for (auto& x: a) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        //res += a[i] * c.z(k, n-k+1) * c.fac[k] * c.fac[n-k-1];
        res += a[i] * c.inv[n-k];
    }
    res *= c.fac[n];
    cout << res;
