...
// superset mobius transform/inversion
// let f[d] = #{seq}d=gcd
// F[d] = #{seq} s.t. d|gcd = MT (f). i.e. f=MI (F). f[d] = sum_{d|g} mu(g/d) F[g]. answer is d=1
// F[d] is easy to compute. := 2^{cnt[d]} - 1
// 
// basically similar problem is like: we wanna f[d] s.t. d=gcd, transform to calc F[d] s.t. d|gcd
void solve() {
    Sieve s(N);
    int n; cin >> n;
    vector<int> cnt(N);
