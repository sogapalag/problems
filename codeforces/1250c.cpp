...
void solve() {
    ...
    cin >> n >> K;
    vector<pi> a(n);
    vector<vector<pli>> evs(N+1);
    for (int i = 0; i < n; i++) {
        int x,y; ll z;
        cin >> x >> y >> z;
        x--;
        evs[y].emplace_back(z, x);
        a[i] = {x,y};
    }
    ...
    SegLazy<pli, ll> rmx (leaves, {-1e18, 0}, 0,
            [](pli u, pli v){ return max(u,v); },
            [](pli& u, ll x){ u.first += x; },
            [](ll& x, ll y){ x += y; }
            );
    ll best = 0;
    int L, R;
    ...
    for (int r = 1; r <= N; r++) {
        for (auto& _: evs[r]) {
            ll p; int l;
            tie(p, l) = _;
            rmx.update(0, l+1, p); // L<=l seg contain it.
        }
        ll mx; int l;
        tie(mx, l) = rmx.query(0, r);
        mx -= K*r;
        if (mx > best) {
            best = mx;
            L = l; R = r;
    ...
    if (!best) cout << 0;
        ...
        for (int i = 0; i < n; i++) {
            int l,r;
            tie(l,r) = a[i];
            if (L <= l && r <= R) res.push_back(i);
        }
        cout << best << ' ' << L+1 << ' ' << R << ' ' << res.size() << '\n';
        for (auto& x: res) {
            cout << x+1 << ' ';
        }
    }
