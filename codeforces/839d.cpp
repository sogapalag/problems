...
vector<int8_t> mu = Sieve(N).mobius_table(N);
template <typename T>
...
// similar to 803f.
// let n = d[i] := cnt that i|x
// now for size k, coeff = k * C(n,k). sum it = n*2^{n-1}. hint k+(n-k)
// so F[d] = sum_d|g of coeffs
// apply superset mobius. to get f. final answer is sum g*f[g]
void solve() {
    int n; cin >> n;
    vector<int> d(N);
    for (int _ = 0; _ < n; _++) {
    ...
    vector<mint> F(N);
    for (int i = 1; i < N; i++) {
        //F[i] = mint(2).exp(d[i]) - 1;
        if (d[i]) F[i] = d[i]*mint(2).exp(d[i]-1);
    }
    //auto f = MIp(F);
    inplace_MIp(F);
    auto&f = F;
