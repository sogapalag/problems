...
const int N = 3e5+10; 
// key observation: if exist valid, then exist size <= 7. since smallest 7 primes product > N
// proof: for valid set S, x is useful, implies \E p|gcd(S\x), which x doesn't have p. all others does.
//    so each x, doesn't have a unique prime which other does. implies size<=7 
// then same as 803f. transform d=g to d|g problem
void solve() {
    Sieve s(N);
    Comb c(N);
    int n; cin >> n;
