...
// note prime-indep
// let exp-array e[1..n] like: 0,0...1,1,...20,20
// for e[i], let L,R be #left,right, L+R=n-1
// note for a set/sequence, it's optimal choose mid, thus cost is like - - - .. + + +
// so consider when e[i] is + or -.
// for +, sum_{r<l} (R r)(L l) = sum_{l-r>0} (R R-r)(L l) = sum_x>R (L+R, x) = sum_x<L (n-1, x)
// GF's view, (1+x)^L(1+1/x)^R
// similar problem: cf 1264d2
void solve() {
    int n; cin >> n;
    for (int _ = 0; _ < n; _++) {
        int x; cin >> x;
        for (auto& be: s.factor(x)) {
            a[be.first][be.second]++;
        }
    }
    Comb c(n);
    ...
    RASQ<mint> b(n); // overkill, linear work
    for (int p: s.prime) {
        int l = n+1, r = n+1;
        for (int i = 19; i >= 1; i--) {
            l -= a[p][i];
    ...
    for (int i = 1; i <= n; i++) {
        mint cnt = b.query(i);
        mint lx = i-1>0? pref[i-2] : 0;
        mint rx = n-i>0? pref[n-i-1] : 0;
        res += cnt * (lx - rx);
    }
    cout << res;
}
