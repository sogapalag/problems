...
bool occur[N];
vector<int> ds[N];
// editorial: iter g. now suppose all x/=g
// iter x dec. order, while has coprime just check x * stk.back * g, pop.
//    magic work since \E y >= stk.back, which y is coprime to x. so we won't get exceed answer
// below if we mark x's divs, then we can consider g=1 is enough.
void solve() {
    int n; cin >> n;
    ll mx = 0;
    for (int _ = 0; _ < n; _++) {
    ...
    // prep
    auto mu = Sieve(N).mobius_table(N);
    for (int d = 1; d < N; d++) {
        for (int i = d; i < N; i+=d) {
            ds[i].push_back(d);
            // mark div that we don't need iter g as editorial
            occur[d] |= occur[i];
        }
    }
    vector<int> cnt(N);
    auto coprime = [&](int x){
        int res = 0;
        for (int d: ds[x]) res += mu[d] * cnt[d];
        return res > 0;
    };
    auto upd = [&](int x, int val){
        for (int d: ds[x]) cnt[d] += val;
    };
