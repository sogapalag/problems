...
const int N = 1024; 
mint dp[N][N][4]; // till i pos, j goods, x indicate whether used previous two number, i.e. i-1,i

void solve() {
    int n,k;
    cin >> n >> k;
    dp[0][0][0b01] = 1; // cannot use 0
    for (int i = 0; i < n; i++) {
        for (int j = 0; j <= i; j++) {
            for (int x = 0; x < 4; x++) {
                // make i pos good
                for (int y:{0,2}) if(i+1<n||y<2){ // cannot use n+1
                    if (x>>y&1)continue;
                    dp[i+1][j+1][(x|1<<y)>>1] += dp[i][j][x];
                }
                // reserve
    ...
    for (int i = k; i <= n; i++) {
        ...
        mint tmp = 0;
        for (int x = 0; x < 4; x++) {
            tmp += dp[n][i][x] * c.fac[n-i];
        }
        res += sign * c.c(i,k) * tmp;
    }
    cout << res;
}
