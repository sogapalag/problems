...
// conclusion. Eij = k^ {max(x+y-n, gcd(x,y)) - n}, where x=n-i,y=n-j.
// note even i=j, E=k^-i match above formula. proof confusing. TODO
// below calc focus on s=x+y, g=gcd(x,y)
void solve() {
    int n,k;
    cin >> n >> k;
    ...
    for (int g = 1; g < n; g++) {
        // s = vg is only match g > s-n >= 0.
        int v = (n+g-1)/g;
        // since g=gcd(x,y), gcd(x/g,v)=1. move those pair to cnt[g]
        cnt[v*g - n] -= phi[v];
        cnt[g] += phi[v];
        // add #pair i=s-n<0. -1 since phi[1]=1, and x,y!=0, so no such pair
        cnt[g] += sum[v-1] - 1;
    }
    mint ik = mint(1)/k;
    mint pw = ik;
