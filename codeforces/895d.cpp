...
void solve() {
    ...
    Comb c(n);
    mint suff = 1;
    for (int x:cnt)suff *= c.ifac[x];
    int cnt_buf[26];
    memcpy(cnt_buf, cnt, sizeof cnt_buf);
    mint suff_buf = suff;
    mint res = 0;
    // prefix equal to s
    for (int j = i; j < n; j++) {
        char lim = j==i?t[j] : ('a'+26);
        for (char k = s[j]+1; k < lim; k++) {
            int x = k-'a';
            x = cnt[x];
            if (x > 0) {
                res += suff * c.fac[x] * c.ifac[x-1] * c.fac[n-j-1];
            }
        }
        int& x = cnt[s[j]-'a'];
        suff *= c.fac[x];
    ...
    for (int j = i; j < n; j++) {
        ...
        suff *= c.ifac[x];
        if (j+1<n) {
            for (char k = 'a'; k < t[j+1]; k++) {
                int x = k-'a';
                x = cnt[x];
                if (x > 0) {
                    res += suff * c.fac[x] * c.ifac[x-1] * c.fac[n-j-1 -1];// we use nxt, one more
                }
            }
        }
    }
